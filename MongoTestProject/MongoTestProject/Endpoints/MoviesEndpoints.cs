...
namespace MongoTest.WebApi.Endpoints
{
    public static class MoviesEndpoints
    {
        ...
        public static void RegisterMoviesEndpoints(this IEndpointRouteBuilder routes)
        {
            ...
            movies.MapGet("director/{director}", getMoviesByDirector).WithTags(TagName);
            ...
            movies.MapGet("genre/{genre}/rating/{rating}", getMoviesInGenreWithRatingHigherThan).WithTags(TagName);

            var getTopViewedMovies = (int amount, IMovieService movieService) => movieService.GetTopViewedMovies(amount);
            movies.MapGet("top/{amount}", getTopViewedMovies).WithTags(TagName);
        }
    }
}
